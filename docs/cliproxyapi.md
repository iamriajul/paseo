...
- Codex sets `model_provider` to `cliproxyapi` only for a slug CLIProxyAPI advertised and Codex itself did not already list. The process still receives `OPENAI_API_KEY`, because `~/.codex/config.toml` may already point `model_provider` at CLIProxyAPI and Codex requires that key. OpenCode and OMP were already per model: CLIProxyAPI rows are `cliproxyapi/<slug>` and `litellm/<slug>`, and the other providers are not rerouted.
