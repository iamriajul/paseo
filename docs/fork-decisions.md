...
a live user_message that acknowledges a pending submission is flushed immediately; disconnect flushes those pending events before clearing turn liveness so a websocket drop cannot strand the working indicator. The live clientMessageId still settles the submission when sequencing drops the row (gap/stale), the viewed timeline is detached, or apply inserts because the snapshot missed the optimistic row. An unreconciled local row also settles by matching text when the live event omits clientMessageId. Disconnect always clears turn liveness even if flush throws.
