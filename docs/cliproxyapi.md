...
- Claude injection is per model. A session gets `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN` only when CLIProxyAPI advertised that id. A manifest model it did not list, including the 200k row and the `[1m]` variant, launches with the local Claude Code login. A hand-added custom model it did not list does the same.
